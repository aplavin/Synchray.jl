@testitem "Conical jet geometry + scalings" begin
	...
	jet = S.ConicalJet(;
		...
		φj=0.05,
		s=1e-3..5,
		ne=S.PowerLaw(-2; val0=2, s0),
		B=S.BFieldSpec_OLD(S.PowerLaw(-1; val0=3, s0), S.ScalarField(), b -> S.FullyTangled(b)),
...
@testitem "Unitful boundary API (ConicalJet)" begin
	...
	jet = S.withunits(S.ConicalJet;
		...
		B=S.BFieldSpec_OLD(S.PowerLaw(-1; val0=3u"Gauss", s0=1u"pc"), S.ScalarField(), b -> S.FullyTangled(b)),
	...
	@test jet.model.Cj_ordered * jet.model.sinavg_j ≈ 1.7e-18 rtol=1e-3
	@test jet.model.Ca_ordered * jet.model.sinavg_a ≈ 7.20e13 rtol=1e-3
