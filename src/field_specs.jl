...
When used in `SynchrotronEmission`, the resulting field is interpreted as the
**comoving-frame** (plasma rest frame) magnetic field.
...
# Time-independence trait: AND of components.
@inline is_time_independent(b::BFieldSpec) =
    is_time_independent(b.scale) && is_time_independent(b.dir) && is_time_independent(b.wrap)
@inline is_time_independent(v::VelocitySpec) =
    is_time_independent(v.dir) && is_time_independent(v.scale)
@inline is_time_independent(v::CombinedVelocity) =
    is_time_independent(v.v1) && is_time_independent(v.v2)
