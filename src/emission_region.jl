...
# GR velocity types (Keplerian, ZAMO, SubKeplerian): four_velocity defined in SynchrayGR.
...
# Time-independence trait: AND of components. `velocity = nothing` (worldline-based)
# defers to the geometry, which is itself time-dependent if it carries a worldline.
@inline is_time_independent(region::EmissionRegion) =
    is_time_independent(region.geometry) && is_time_independent(region.velocity) && is_time_independent(region.emission)
@inline is_time_independent(::Nothing) = true
