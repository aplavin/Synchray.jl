...
Singleton worldline: position fixed at the spatial origin, 4-velocity at rest in
the lab frame. Used as `Ball.center` to express a static spherical emission
region centered at the origin, with `+ẑ` as the conventional polar axis.
"""
struct StationaryAtOrigin end
...
"""
    GeometryUnion{T <: Tuple} <: AbstractGeometry

Union of multiple geometries. A point is inside if it is inside any component.
"""
struct GeometryUnion{T <: Tuple} <: AbstractGeometry
    geometries::T
end
GeometryUnion(geoms...) = GeometryUnion(geoms)
...
# Worldline-based geometries depend on x4.t (their is_inside reads it via x4 - g.center.x0).
@inline is_time_independent(::Geometries.Ellipsoid) = false
@inline is_time_independent(::Geometries.Ball) = false
@inline is_time_independent(::Geometries.Ball{Geometries.StationaryAtOrigin}) = true

@inline function geometry_axis(g::Geometries.Ball{Geometries.StationaryAtOrigin})
	T = eltype(g.size)
	return SVector(zero(T), zero(T), oneunit(T))
end

prepare_for_computations(g::Geometries.Ball) = g

@inline four_velocity(g::Geometries.Ball, x4) = four_velocity(g.center)
@inline four_velocity(g::Geometries.Ball{Geometries.StationaryAtOrigin}, x4) =
	FourVelocity(one(eltype(x4)), zero(@swiz x4.xyz))
...
@inline is_inside(g::Geometries.Ball{Geometries.StationaryAtOrigin}, x4) =
	x4.x^2 + x4.y^2 + x4.z^2 ≤ g.size^2
...
@inline function z_interval(g::Geometries.Ball{Geometries.StationaryAtOrigin}, ray::Ray)
	r0 = @swiz ray.x0.xyz
	n̂  = direction3(ray)
	b  = dot(n̂, r0)
	c  = dot(r0, r0) - g.size^2
	D  = b^2 - c
	if !(D > 0)
		s0 = zero(D)
		return s0 .. (s0 - eps(oneunit(s0)))
...
@inline function natural_coords(g::Geometries.Ball{Geometries.StationaryAtOrigin}, x4)
	x, y, z = x4.x, x4.y, x4.z
	r = sqrt(x^2 + y^2 + z^2)
	θ = atan(sqrt(x^2 + y^2), z)
	return (; r, θ)
end

@inline function rotation_local_to_lab(g::Geometries.Ball{Geometries.StationaryAtOrigin})
	T = eltype(g.size)
...
"""
	ray_in_lab_coords(ray::Ray; s_range) -> StructArray{SVector{3}}

Compute the two endpoints of a ray segment in lab coordinates.
"""
function ray_in_lab_coords(ray::Ray; s_range)
	n̂ = direction3(ray)
	r0 = @swiz ray.x0.xyz
	map(endpoints(s_range)) do s
		r0 + s * n̂
...
Base.union(g1::Geometries.AbstractGeometry, g2::Geometries.AbstractGeometry, gs::Geometries.AbstractGeometry...) = Geometries.GeometryUnion(g1, g2, gs...)
